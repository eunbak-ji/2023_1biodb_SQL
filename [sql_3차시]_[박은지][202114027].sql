...
/*(15) Book 테이블에 새로운 도서 ‘스포츠 의학’ 을 삽입하시오. 스포츠 의학은 한솔의학서적에서 출간했으며, 가격은 미정이다.*/
INSERT INTO Book (bookid, bookname, publisher, price)
VALUES (11,'스포츠과학','한솔의학서',NULL);
...
SELECT * FROM Orders GROUP BY custid HAVING MAX(saleprice) < (SELECT AVG(saleprice) FROM Orders);
...
/*(28) Customer 테이블에 새로운 고객 (이름:박세리, 주소:대한민국 부산, 전화번호:없음(null))을 삽입하시오*/
INSERT INTO Customer (custid, name, address, phone)
...
SELECT orderid, custid, MIN(saleprice) FROM Orders GROUP BY custid 
